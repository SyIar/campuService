<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>已接订单</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图 -->
    <el-card>
...
<script>
  export default {
    ...
    created() {
      this.getAcceptedOrdersList()
    },
    methods: {
      async getAcceptedOrdersList() {
        const { data: res } = await this.$http.get('api/order/getOrderByAccepter', { params: { accepterId: window.sessionStorage.getItem('userId') }})
        const { code } = res
        const { data } = res
        if (code !== '200') {
